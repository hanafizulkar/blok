...
import { Search, Shield, ArrowRight, Blocks } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-bansos-bg">
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <p className="text-sm text-bansos-text-muted mb-6">Masukkan NIK, QR Token, atau Tracking ID untuk melihat status bantuan</p>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 max-w-lg mb-8">
          <div className="relative flex-1">
            ...
            <Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Cari..." className="pl-10 bg-bansos-surface border-bansos-border text-bansos-text h-11" />
          </div>
          ...
          <Card key={i} className="bg-bansos-surface border-bansos-border mb-3">
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="font-mono text-sm text-bansos-accent font-semibold">{r.tracking_id}</div>
                  <div className="text-xs text-bansos-text-muted">{r.recipient_name}</div>
                </div>
                <Badge variant="outline" className="text-xs border-bansos-border text-bansos-text-muted capitalize">{r.status}</Badge>
              </div>
              ...
              {r.block_hash && (
                <div className="bg-bansos-bg rounded p-2.5 border border-bansos-border flex items-center gap-2">
                  ...
                  <div className="text-[10px]">
                    ...
                    <code className="text-green-600 dark:text-green-400 font-mono">{r.block_hash}</code>
                  </div>
                </div>
              )}
            </CardContent>
